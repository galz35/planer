...
async function testAliVisibility() {
    ...
    const carnetSolicitante = '402178'; // Ali Rodriguez
    ...
    const sql = `
      ...
      SELECT DISTINCT v.carnet, u."nombreCompleto"
      FROM (
      ...
      LEFT JOIN "p_Usuarios" u ON v.carnet = u.carnet
    ...
    try {
        await ds.initialize();
        const rows = await ds.query(sql, [carnetSolicitante]);
        rows.forEach(r => {
            console.log(`RESULT: ${r.carnet} | ${r.nombreCompleto}`);
        });
