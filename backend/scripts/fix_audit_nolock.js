...
async function updateProceduresNoLock() {
    try {
        const pool = await sql.connect(config);

        console.log('Updating SP with WITH (NOLOCK) to prevent read blocks...');
        await pool.request().query(`
            ...
            BEGIN
                SET NOCOUNT ON;
                SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; -- Force dirty reads for speed
                ...
                FROM dbo.p_Auditoria a WITH (NOLOCK)
                INNER JOIN #Equipo e ON e.carnet = a.carnet
                LEFT JOIN dbo.p_Usuarios u WITH (NOLOCK) ON a.carnet = u.carnet
                WHERE
                    @searchTerm IS NULL
