...
export class AccesoService {
    ...
    async crearPermisoArea(dto: CrearPermisoAreaDto) {
        const empleadoRecibe = await accesoRepo.buscarUsuarioPorCarnet(dto.carnetRecibe);
        if (!empleadoRecibe) throw new BadRequestException(`Empleado receptor no encontrado: ${dto.carnetRecibe}`);

        const nodo = await accesoRepo.buscarNodoPorId(Number(dto.idOrgRaiz));
    ...
    async crearPermisoEmpleado(dto: CrearPermisoEmpleadoDto) {
        ...
        const empleadoRecibe = await accesoRepo.buscarUsuarioPorCarnet(carnetRecibe);
        if (!empleadoRecibe) throw new BadRequestException(`Empleado receptor no encontrado: ${carnetRecibe}`);

        const empleadoObjetivo = await accesoRepo.buscarUsuarioPorCarnet(carnetObjetivo);
    ...
    async previewEmpleadosPorNodo(idOrgRaiz: string, alcance: 'SUBARBOL' | 'SOLO_NODO' = 'SUBARBOL'): Promise<any> {
        if (alcance === 'SOLO_NODO') {
            const muestra = await accesoRepo.obtenerEmpleadosNodoDirecto(idOrgRaiz, 50);
