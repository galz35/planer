...
export class AdminSecurityController {
    ...
    private checkAdminAccess(user: any) {
        if (!['Admin', 'Administrador', 'SuperAdmin'].includes(user.rol || '')) {
            throw new InsufficientPermissionsException('Solo administradores pueden acceder a esta función');
        }
    }

    @Get('users-access')
    @ApiOperation({ summary: 'Obtener lista de usuarios con información de acceso' })
    ...
    async assignMenu(@Body() dto: AssignMenuDto, @Request() req) {
        this.checkAdminAccess(req.user);
        await this.securityService.assignCustomMenu(dto.idUsuario, dto.customMenu ?? null);
