...
export class ImportController {
    ...
    getEmpleadosTemplate() {
        return {
            ...
            data: {
                ...
                campos: [
                    { nombre: 'nombre', tipo: 'string', requerido: true, ejemplo: 'JUAN CARLOS PEREZ GOMEZ' },
                    { nombre: 'correo', tipo: 'string', requerido: true, ejemplo: 'juan.perez@claro.com.ni' },
                    { nombre: 'telefono', tipo: 'string', requerido: false, ejemplo: '88881234' },
                    { nombre: 'activo', tipo: 'boolean', requerido: false, ejemplo: true, default: true },
                    { nombre: 'nodoNombre', tipo: 'string', requerido: false, ejemplo: 'NI GERENCIA DE RECURSOS HUMANOS' },
                    { nombre: 'rolEnNodo', tipo: 'string', requerido: false, ejemplo: 'Colaborador', opciones: ['Lider', 'Colaborador', 'Miembro'] },
                ],
                ejemplo: [
                    {
    ...
    getOrganizacionTemplate() {
        return {
            success: true,
            data: {
                descripcion: 'Estructura para importar nodos de organización desde Excel/JSON',
                campos: [
                    { nombre: 'nombre', tipo: 'string', requerido: true, ejemplo: 'NI SUBGERENCIA DE CAPACITACION' },
                    { nombre: 'tipo', tipo: 'string', requerido: true, opciones: ['Dirección', 'Gerencia', 'Subgerencia', 'Equipo'] },
                    { nombre: 'nombrePadre', tipo: 'string', requerido: false, ejemplo: 'NI GERENCIA DE RECURSOS HUMANOS' },
                    { nombre: 'activo', tipo: 'boolean', requerido: false, default: true },
                ],
                ejemplo: [
                    {
     ...
     */
    @Post('empleados')
    async importEmpleados(@Body() body: { empleados: ImportEmpleadoDto[], modo?: 'crear' | 'actualizar' | 'upsert' }): Promise<ImportResult> {
        const { empleados, modo = 'upsert' } = body;

        if (!empleados || !Array.isArray(empleados) || empleados.length === 0) {
            throw new HttpException('Se requiere un array de empleados', HttpStatus.BAD_REQUEST);
        }

        if (empleados.length > 5000) {
            throw new HttpException('Máximo 5000 empleados por lote', HttpStatus.BAD_REQUEST);
        }
     ...
     */
    @Post('organizacion')
    async importOrganizacion(@Body() body: { nodos: ImportNodoDto[], modo?: 'crear' | 'actualizar' | 'upsert' }): Promise<ImportResult> {
        const { nodos, modo = 'upsert' } = body;

        if (!nodos || !Array.isArray(nodos) || nodos.length === 0) {
            throw new HttpException('Se requiere un array de nodos', HttpStatus.BAD_REQUEST);
        }

        if (nodos.length > 1000) {
            throw new HttpException('Máximo 1000 nodos por lote', HttpStatus.BAD_REQUEST);
        }
     ...
     */
    @Post('asignaciones')
    async importAsignaciones(@Body() body: { asignaciones: { correo: string, nodoNombre: string, rol: string }[] }): Promise<ImportResult> {
        const { asignaciones } = body;

        if (!asignaciones || !Array.isArray(asignaciones) || asignaciones.length === 0) {
            throw new HttpException('Se requiere un array de asignaciones', HttpStatus.BAD_REQUEST);
        }
