...
describe('AuthService', () => {
    ...
    describe('validateUser', () => {
        ...
        it('should return user when credentials are valid', async () => {
            (authRepo.obtenerUsuarioPorIdentificador as jest.Mock).mockResolvedValue(mockUser);
            (authRepo.obtenerCredenciales as jest.Mock).mockResolvedValue(mockCreds);
            (authRepo.actualizarUltimoLogin as jest.Mock).mockResolvedValue(undefined);
            (bcrypt.compare as jest.Mock).mockResolvedValue(true);

            const result = await service.validateUser('test@example.com', 'password123');

            expect(result).toEqual(mockUser);
            expect(authRepo.obtenerUsuarioPorIdentificador).toHaveBeenCalledWith('test@example.com');
            expect(bcrypt.compare).toHaveBeenCalledWith('password123', 'hashedPassword123');
        });

        it('should return null when user does not exist', async () => {
            (authRepo.obtenerUsuarioPorIdentificador as jest.Mock).mockResolvedValue(null);
        ...
        it('should return null for inactive user', async () => {
            (authRepo.obtenerUsuarioPorIdentificador as jest.Mock).mockResolvedValue(null); // Repo filter should handle it
    ...
    describe('login', () => {
        ...
        it('should return tokens and user info on successful login', async () => {
            ...
            (authRepo.actualizarRefreshToken as jest.Mock).mockResolvedValue(undefined);
    ...
    describe('refreshTokens', () => {
        ...
        it('should throw UnauthorizedException when no refresh token stored', async () => {
            (authRepo.obtenerCredenciales as jest.Mock).mockResolvedValue({ idUsuario: 1, refreshTokenHash: null });
