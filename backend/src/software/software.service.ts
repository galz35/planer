...
export class SoftwareService {
    ...
    async getDashboardStats(userId: number, month: number, year: number) {
        try {
            ...
            const visibleUsers = await this.visibilidadService.obtenerEmpleadosVisibles(user.carnet);
            const ids = visibleUsers.length > 0 ? visibleUsers.map(u => u.idUsuario) : [userId];
            ...
            const projectsStats = projectsRaw.map(p => {
                ...
                return {
                    ...
                    progress: p.teamTasksCount > 0 ? Math.round((p.teamTasksDone / p.teamTasksCount) * 100) : 0
                };
            });
            ...
            tasksRaw.forEach(t => {
                ...
                const current = hierarchyMap.get(key) || {
                    name: key, hechas: 0, atrasadas: 0, enCurso: 0, bloqueadas: 0, pendientes: 0, total: 0
                };
            ...
            const globalCompletion = totalTeamTasks > 0 ? Math.round((teamTasksDone / totalTeamTasks) * 100) : 0;

            return {
                globalCompletion,
                visibleTeamCount: visibleUsers.length,
                totalActiveProjects: projectsStats.filter(p => p.totalTasks > 0).length,
                topDelays,
                projectsStats,
                blockersDetail,
